...
トラックボールを動かすとMOUSEレイヤー（レイヤー1）に自動切替されます。

#### レイヤー解除条件
- **マウスキー以外のキーを押す** → 即座にデフォルトレイヤーに戻る
- **マウスキーを押す** → マウスレイヤーに留まる
- **100秒間操作なし** → タイムアウトで戻る（実質無効）

#### マウスキーとして扱われるキー（excluded-positions）
MOUSEレイヤーで`&trans`と`&to 0`以外に設定されているキーは、自動的にマウスキーとして扱われます。

**自動検出される例:**
- `mkp MB1/MB2/MB3` - マウスボタン
- `msc SCRL_UP/DOWN` - スクロール
- `mo 2` - SCROLLレイヤーへの一時切替
- `kp LC(C)`, `kp LC(V)` - コピー/ペースト
- `kp BACKSPACE` - バックスペース

これらのキーを押してもマウスレイヤーを維持します。

**注:** excluded-positionsはビルド時に自動更新されます。MOUSEレイヤーにキーを追加するだけで自動的に反映されます。
